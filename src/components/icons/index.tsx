import { cn } from "@/lib/utils";

type IconProps = { className?: string; size?: number };

const defaults = { size: 16 };

export function HeartIcon({ className, size = defaults.size }: IconProps) {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 32 32"
      fill="none"
      className={cn("stroke-current", className)}
      strokeWidth={2}
    >
      <path d="M16 28c7-4.35 10-7.5 10-12.18a5.5 5.5 0 0 0-9.71-3.56 1 1 0 0 1-1.58 0A5.5 5.5 0 0 0 6 15.82C6 20.5 9 23.65 16 28z" />
    </svg>
  );
}

export function HeartFilledIcon({ className, size = defaults.size }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" className={cn("fill-brand", className)}>
      <path d="M16 28c7-4.35 10-7.5 10-12.18a5.5 5.5 0 0 0-9.71-3.56 1 1 0 0 1-1.58 0A5.5 5.5 0 0 0 6 15.82C6 20.5 9 23.65 16 28z" />
    </svg>
  );
}

export function SearchIcon({ className, size = defaults.size }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="currentColor" className={className}>
      <path d="M13 4a9 9 0 1 0 5.74 16.06l7.18 7.18 1.41-1.41-7.18-7.18A9 9 0 0 0 13 4zm0 2a7 7 0 1 1 0 14 7 7 0 0 1 0-14z" />
    </svg>
  );
}

export function StarIcon({ className, size = 12 }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="currentColor" className={className}>
      <path d="M16 2l4.09 8.28 9.15 1.33-6.62 6.45 1.56 9.11L16 22.77l-7.18 3.77 1.56-9.11-6.62-6.45 9.15-1.33L16 2z" />
    </svg>
  );
}

export function GlobeIcon({ className, size = defaults.size }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="currentColor" className={className}>
      <path d="M16 2C8.27 2 2 8.27 2 16s6.27 14 14 14 14-6.27 14-14S23.73 2 16 2zm0 2c1.93 0 4.57 3.5 5.47 9H10.53C11.43 7.5 14.07 4 16 4zM6.07 16c0-1.1.18-2.16.5-3.14h5.93v3.14H6.07zm2.43 6.14c.32-.98.5-2.04.5-3.14h6.43v6.28c-3.5-.68-6.14-2.14-6.93-3.14zm8.5 3.14v-6.28h6.43c-.79 1-3.43 2.46-6.93 3.14zm8.5-9.28h5.93c.32.98.5 2.04.5 3.14s-.18 2.16-.5 3.14h-5.93v-6.28zm0-2h5.43c.32-.98.5-2.04.5-3.14 0-1.1-.18-2.16-.5-3.14H25.5v6.28z" />
    </svg>
  );
}

export function MenuIcon({ className, size = defaults.size }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="currentColor" className={className}>
      <path d="M4 8h24v2H4V8zm0 7h24v2H4v-2zm0 7h24v2H4v-2z" />
    </svg>
  );
}

export function UserIcon({ className, size = defaults.size }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="currentColor" className={className}>
      <path d="M16 16a5 5 0 1 0 0-10 5 5 0 0 0 0 10zm0 2c-5.33 0-8 2.67-8 4v2h16v-2c0-1.33-2.67-4-8-4z" />
    </svg>
  );
}

export function ChevronLeftIcon({ className, size = defaults.size }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="currentColor" className={className}>
      <path d="M20.03 4.97L9 16l11.03 11.03 1.41-1.41L11.83 16l9.61-9.62-1.41-1.41z" />
    </svg>
  );
}

export function ChevronRightIcon({ className, size = defaults.size }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="currentColor" className={className}>
      <path d="M11.97 4.97L23 16 11.97 27.03l-1.41-1.41L20.17 16 10.56 6.38l1.41-1.41z" />
    </svg>
  );
}

export function CloseIcon({ className, size = defaults.size }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="currentColor" className={className}>
      <path d="M6.4 4.93L4.93 6.4 14.53 16l-9.6 9.6 1.47 1.47L16 17.47l9.6 9.6 1.47-1.47L17.47 16l9.6-9.6-1.47-1.47L16 14.53 6.4 4.93z" />
    </svg>
  );
}

export function ShareIcon({ className, size = defaults.size }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="currentColor" className={className}>
      <path d="M24 4l-8 8V9.5C9.5 9.5 5 12 5 18c0 0 3-4 9-4.5V20l8 8V4z" />
    </svg>
  );
}

export function FilterIcon({ className, size = defaults.size }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="currentColor" className={className}>
      <path d="M4 6h24v2H4V6zm4 9h16v2H8v-2zm4 9h8v2h-8v-2z" />
    </svg>
  );
}

export function MapIcon({ className, size = defaults.size }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="currentColor" className={className}>
      <path d="M10 4L2 7v21l8-3 12 3 8-3V7l-8 3-12-3zm0 3.18l10 2.5v16.14l-10-2.5V7.18zm12 2.5l6-1.5v16.14l-6 1.5V9.68z" />
    </svg>
  );
}

export function AirbnbLogo({ className }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 100 32"
      fill="currentColor"
      className={cn("h-8 w-auto text-brand", className)}
    >
      <path d="M16.05 2.5c-3.2 0-5.8 2.6-5.8 5.8 0 4.5 5.8 11.5 5.8 11.5s5.8-7 5.8-11.5c0-3.2-2.6-5.8-5.8-5.8zm0 7.9a2.1 2.1 0 1 1 0-4.2 2.1 2.1 0 0 1 0 4.2zM32 2.5h-4.2v17.3H32V2.5zm8.5 0c-4.7 0-8.5 3.8-8.5 8.5v8.8h4.2v-3.5c.9.6 2 1 3.2 1 4.7 0 8.5-3.8 8.5-8.5S45.2 2.5 40.5 2.5zm0 12.8c-2.4 0-4.3-1.9-4.3-4.3s1.9-4.3 4.3-4.3 4.3 1.9 4.3 4.3-1.9 4.3-4.3 4.3zM56.8 2.5c-4.7 0-8.5 3.8-8.5 8.5s3.8 8.5 8.5 8.5 8.5-3.8 8.5-8.5-3.8-8.5-8.5-8.5zm0 12.8c-2.4 0-4.3-1.9-4.3-4.3s1.9-4.3 4.3-4.3 4.3 1.9 4.3 4.3-1.9 4.3-4.3 4.3zM72.5 2.5c-2.3 0-4.2 1.9-4.2 4.2v13.1h4.2V11.5h6.5v8.3H83V2.5h-4.2v5.8h-6.3V2.5h-4.2zm15.3 0v17.3h4.2V2.5h-4.2z" />
    </svg>
  );
}

export function CategoryIcon({ icon, className }: { icon: string; className?: string }) {
  switch (icon) {
    case "all":
      return <span className={cn("text-xl", className)}>🏆</span>;
    case "mansions":
      return <span className={cn("text-xl", className)}>🏛️</span>;
    case "beach":
      return <span className={cn("text-xl", className)}>🏖️</span>;
    case "cabins":
      return <span className={cn("text-xl", className)}>🛖</span>;
    case "homes":
      return <span className={cn("text-xl", className)}>🏠</span>;
    case "experiences":
      return <span className={cn("text-xl", className)}>🎈</span>;
    case "services":
      return <span className={cn("text-xl", className)}>🛎️</span>;
    default:
      return <span className={cn("text-xl", className)}>🏡</span>;
  }
}

export function DestinationIcon({ icon, className }: { icon: string; className?: string }) {
  const icons: Record<string, string> = {
    nearby: "🧭",
    "cape-cod": "🏡",
    boston: "🏙️",
    portland: "🌲",
    orlando: "🎢",
    beach: "🏖️",
  };
  return (
    <span className={cn("flex size-12 items-center justify-center rounded-xl bg-bg-subtle text-2xl", className)}>
      {icons[icon] ?? "📍"}
    </span>
  );
}
