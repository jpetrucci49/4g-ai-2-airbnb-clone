...
export function PhotoCarousel({ images, title }: PhotoCarouselProps) {
  ...
  return (
    <div id="room-photos" className="relative aspect-[4/3] md:hidden">
      ...
      <button type="button" onClick={prev} className="absolute left-3 top-1/2 flex size-7 -translate-y-1/2 items-center justify-center rounded-full bg-white/90 shadow" aria-label="Previous">
        <ChevronLeftIcon size={14} />
      </button>
      <button type="button" onClick={next} className="absolute right-3 top-1/2 flex size-7 -translate-y-1/2 items-center justify-center rounded-full bg-white/90 shadow" aria-label="Next">
